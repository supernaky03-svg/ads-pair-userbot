...
def _normalize_database_url(url: str) -> str:
    # Neon often provides postgres:// or postgresql:// URLs. SQLAlchemy async needs asyncpg.
    if url.startswith("postgres://"):
        url = "postgresql://" + url[len("postgres://") :]
    if url.startswith("postgresql://"):
        url = "postgresql+asyncpg://" + url[len("postgresql://") :]

    # asyncpg does not use libpq's sslmode name. Convert sslmode=require to ssl=true.
    parts = urlsplit(url)
